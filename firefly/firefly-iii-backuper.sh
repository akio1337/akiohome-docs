...
backup () {
    ...
    if [ -z $db_container ]; then
        ...
        docker exec $db_container bash -c '/usr/bin/mariadb-dump -u root --password="PASSWORD" firefly ' > "$dest_path/tmp/firefly_db.sql"
...
restore () {
    ...
    if [ -z $db_container ]; then
        ...
        cat "$src_path/tmp/firefly_db.sql" | docker exec -i $db_container bash -c '/usr/bin/mariadb -u root --password="PASSWORD" "firefly"'
